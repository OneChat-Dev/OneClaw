...
If `npx` is **not found**, fall back to the bundled bun shipped with OneClaw.
OneClaw sets the `CHERRY_STUDIO_BUN_PATH` environment variable pointing to its
...
if [ -n "$CHERRY_STUDIO_BUN_PATH" ] && [ -x "$CHERRY_STUDIO_BUN_PATH" ]; then
  ...
  echo "Error: Neither npx nor bundled bun found. Install Node.js or run OneClaw's bun installer."
fi
```
