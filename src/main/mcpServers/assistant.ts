...
const NAVIGATE_TOOL: Tool = {
  name: 'navigate',
  description: 'Navigate OneClaw to a specific page. Refer to the route table in your skills for available paths.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
...
const DIAGNOSE_TOOL: Tool = {
  name: 'diagnose',
  description:
    'Read OneClaw runtime state for troubleshooting. Use this to inspect app info, provider config, connectivity, logs, and MCP server status.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
  ...
  private async checkUpdate() {
    try {
      ...
      const response = await fetch('https://api.github.com/repos/CherryHQ/cherry-studio/releases/latest', {
        method: 'GET',
        headers: { Accept: 'application/vnd.github.v3+json', 'User-Agent': 'OneClaw' },
        signal: controller.signal
      })
      clearTimeout(timeout)
