...
export const listKnowledgeBases = async (req: ValidationRequest, res: Response): Promise<Response> => {
  try {
    ...
    //           Redux access requires OneClaw window to be open.
    ...
    try {
      ...
      if (isReduxUnavailableError(error)) {
        ...
        return res.status(503).json({
          error: {
            message: 'Knowledge bases are only available when OneClaw window is open',
  ...
  try {
    ...
    if (!bases || bases.length === 0) {
      return res.json({
        ...
        total: 0,
        searched_bases: [],
        warnings: ['No knowledge bases configured. Please add knowledge bases in OneClaw.']
      })
    }
