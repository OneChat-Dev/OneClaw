---
name: cherry-pr-test
description: Test OneClaw PRs by checking out the branch, launching the Electron app in debug mode, and running interactive UI tests via CDP.
---

# OneClaw PR Test

Automated PR testing workflow for OneClaw. Checks out a PR, launches
...
- Always kill existing OneClaw processes before launching a new instance.
...
- A PR URL (e.g., `https://github.com/OneChat-Dev/OneClaw/pull/13955`)
...
1. **Kill any existing OneClaw processes** (graceful SIGTERM first):
   ```bash
...
2. **Verify connection and identify the main page**:
   ...
   agent-browser tab
   ```
   You should see the main OneClaw page at `http://localhost:5173/`.
...
1. **Kill all OneClaw processes** (graceful SIGTERM first):
   ```bash
   pkill -f "cherry-studio.*Electron" 2>/dev/null
   pkill -f "electron-vite" 2>/dev/null
   lsof -ti :9222 | xargs kill 2>/dev/null
